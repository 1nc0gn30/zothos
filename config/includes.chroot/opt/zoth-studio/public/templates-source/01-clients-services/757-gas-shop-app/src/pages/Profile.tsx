...
import { Check, Mail, Save, User as UserIcon, CircleAlert, DollarSign, ToggleRight } from 'lucide-react';
import HelpTip from '../components/HelpTip';
import LinkingGate from '../components/LinkingGate';
...
export default function ProfilePage() {
  const { profile, fetchProfile, user, hasSolanaIdentity, solanaProviderId, advancedMode, toggleAdvancedMode } = useAuthStore();
  ...
  setCashappTag(profile.cashapp_tag ?? '');
}, [profile, user]);
    ...
    try {
      const { error: updateError } = await supabase
        ...
        .update({
          ...
          cashapp_tag: cashappTag.trim() || null,
        })
        .eq('id', user.id);
  ...
  return (
    ...
    >
      <section className="hero-watermark-wrap bg-zinc-950 text-white relative overflow-hidden rounded-[1.8rem] p-6 sm:p-8 shadow-xl border border-zinc-800">
        <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[150%] bg-primary/20 rounded-full blur-[80px] pointer-events-none" />
        <LottieWatermark src="/lottie/smoke-spark.json" className="lottie-watermark--hero" />
        
        <div className="hero-watermark-content grid gap-6 xl:grid-cols-[1fr_0.95fr] xl:items-start">
          <div className="min-w-0">
            <div className="flex items-center gap-5">
              ...
              <div className="min-w-0">
                ...
                <h1 className="mt-1 truncate font-display text-3xl font-extrabold tracking-tight sm:text-4xl">{username || 'Gas member'}</h1>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              {advancedMode && (
                <span className={`inline-flex max-w-full items-center truncate rounded-full px-3 py-1 text-xs font-bold ${hasSolanaIdentity ? 'bg-green-500/15 text-green-400' : 'bg-amber-500/15 text-amber-300'}`}>
                  {hasSolanaIdentity ? 'Solana linked' : 'Solana not linked'}
                </span>
              )}
          ...
          <div className="grid gap-4 sm:grid-cols-3 xl:grid-cols-3">
            <article className="bg-zinc-900/80 border border-zinc-800 rounded-[1.35rem] p-5 backdrop-blur-sm">
              ...
              <div className="mt-2 text-2xl font-extrabold text-white">{Number(profile.wallet_balance ?? 0).toFixed(2)}</div>
            ...
            <article className="bg-zinc-900/50 border border-zinc-800 rounded-[1.35rem] p-5 backdrop-blur-sm">
              ...
              <div className="mt-2 text-2xl font-extrabold text-white">{selectedPreferenceCount}</div>
            ...
            <article className="bg-zinc-900/50 border border-zinc-800 rounded-[1.35rem] p-5 backdrop-blur-sm">
              ...
              <div className="mt-2 truncate text-lg font-extrabold text-white">{profile.id.slice(0, 8).toUpperCase()}</div>
      ...
      {!advancedMode ? (
        <div className="mt-8 rounded-[1.35rem] border border-border bg-card p-5 shadow-sm">
          <div className="flex items-start gap-3">
            <ToggleRight size={20} className="shrink-0 text-primary mt-0.5" />
            <div className="min-w-0">
              <h3 className="text-sm font-bold text-foreground">Advanced user mode</h3>
              <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
                Enable advanced mode to link a Solana wallet and use DePay for credit top-ups. Cash App works without this.
              </p>
              <button
                type="button"
                onClick={toggleAdvancedMode}
                className="mt-3 inline-flex items-center gap-2 rounded-xl border border-primary/30 bg-primary/10 px-4 py-2 text-sm font-bold text-primary transition-all hover:bg-primary/20"
              >
                <ToggleRight size={16} />
        ...
        <div className="mt-8 rounded-[1.35rem] border border-border bg-card p-5 shadow-sm">
          <div className="flex items-start gap-3">
            <ToggleRight size={20} className="shrink-0 text-primary mt-0.5" />
            <div className="min-w-0">
              <h3 className="text-sm font-bold text-foreground">Advanced user mode enabled</h3>
              <p className="text-sm text-muted-foreground mt-1 leading-relaxed">
                You can see DePay options and link a Solana wallet. Disable this to hide crypto features and keep the simple Cash App flow.
              </p>
              <button
                type="button"
                onClick={toggleAdvancedMode}
                className="mt-3 inline-flex items-center gap-2 rounded-xl border border-zinc-700 bg-zinc-900/80 px-4 py-2 text-sm font-bold text-zinc-300 transition-all hover:bg-zinc-800 hover:text-white"
              >
                <ToggleRight size={16} />
      ...
      <section className="mt-8 bg-card border border-border rounded-[1.8rem] p-6 sm:p-8 shadow-sm">
        {advancedMode && hasSolanaIdentity && (
          <div className="mb-8 rounded-[1.35rem] border border-green-500/20 bg-green-500/10 p-5">
            <div className="text-xs font-bold uppercase tracking-wider text-green-500">Verified Solana wallet</div>
            <p className="mt-2 truncate text-sm font-semibold text-foreground font-mono">
              {solanaProviderId ?? 'Linked'}
            </p>
            <p className="mt-2 text-xs leading-relaxed text-muted-foreground">
              Cryptographically verified and linked. You are ready to add credits (via DePay or Cash App) and order.
            </p>
          </div>
        )}
          ...
          <div className="grid grid-cols-3 gap-4 sm:flex sm:flex-wrap">
            {AVATAR_OPTIONS.map((option) => (
              <button
                ...
                className={`relative min-w-0 rounded-[1.35rem] border-2 p-3 text-center transition-all duration-300 ${
          ...
          <div className="min-w-0 space-y-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="input-premium pl-11 pr-4 py-3 font-medium"
        ...
        <div className="mt-6 min-w-0 space-y-3">
          <div className="flex items-center gap-2">
            <label className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Cash App $Cashtag</label>
            <HelpTip text="Add your $Cashtag so we can verify Cash App transfers and process refunds faster." />
          </div>
          <div className="relative">
          ...
          <p className="text-xs text-muted-foreground">
            Recommended if you plan to pay with Cash App — it speeds up refunds and payment matching.
          </p>
        </div>
              ...
              return (
                <button
                  ...
                  className={`rounded-full border-2 px-5 py-2.5 text-sm font-bold capitalize transition-all ${
                    isActive
                      ? 'border-primary bg-primary/10 text-primary shadow-sm glow-primary'
        ...
        <div className="mt-8 pt-6 border-t border-border">
          <button 
            ...
            onClick={handleSave} 
            disabled={saving} 
            className="inline-flex items-center justify-center gap-2 btn-premium text-primary-foreground px-8 py-3.5 rounded-xl font-bold disabled:opacity-50 w-full sm:w-auto"
          >
            <Save size={18} />
