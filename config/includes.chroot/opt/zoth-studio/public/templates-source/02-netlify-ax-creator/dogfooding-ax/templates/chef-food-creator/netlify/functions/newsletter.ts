...
export const handler = async (event: any) => {
  ...
  try {
    if (process.env.BEEHIIV_API_KEY && process.env.BEEHIIV_PUBLICATION_ID) {
      ...
      return { statusCode: 200, headers: CORS_HEADERS, body: JSON.stringify({ success: true, message: 'You’re on the kitchen crew. Check your inbox.' }) };
    }

    return {
      statusCode: 200,
      headers: CORS_HEADERS,
      body: JSON.stringify({ success: true, message: 'You’re on the kitchen crew. Check your inbox.' }),
    };
  } catch (err: any) {
