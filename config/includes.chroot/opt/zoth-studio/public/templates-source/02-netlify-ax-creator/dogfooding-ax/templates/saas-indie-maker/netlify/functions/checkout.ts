...
export const handler = async (event: any) => {
  ...
  const returnUrl = `${process.env.SITE_URL || 'http://localhost:3473'}${body.return_path || '/deal'}`;
  ...
  try {
    if (!process.env.LS_API_KEY || !process.env.LS_STORE_ID || !process.env.LS_VARIANT_ID) {
      return {
        ...
        headers: CORS,
        body: JSON.stringify({
          error: 'LemonSqueezy not configured. Set LS_API_KEY, LS_STORE_ID, and LS_VARIANT_ID in Netlify env.',
        }),
      };
    }
