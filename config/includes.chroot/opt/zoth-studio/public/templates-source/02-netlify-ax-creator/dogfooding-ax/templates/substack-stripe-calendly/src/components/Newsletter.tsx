...
export function Newsletter() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus('success');
      setMessage(data.message || 'Redirecting to Substack…');
      if (data.url) {
    ...
    <section id="newsletter" className="py-20 px-6 bg-zinc-900">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Join the newsletter</h2>
          <p className="text-zinc-400">One short email each week on writing, publishing, and building a readership.</p>
        </div>
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3">
          ...
          >
            {status === 'loading' ? 'Redirecting…' : 'Subscribe on Substack'}
          </button>
        </form>
