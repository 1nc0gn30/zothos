...
export function Newsletter() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus('success');
      setMessage(data.message || 'You’re on the crew roster. Check your inbox.');
      setEmail('');
    } catch (err: any) {
      setStatus('error');
      ...
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          <span className="inline-block px-3 py-1 rounded-full bg-brand-900/40 text-brand-400 text-sm font-medium mb-4">
            Free weekly recipe letter
          </span>
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Join the kitchen crew</h2>
          <p className="text-zinc-400">One email a week with a tested recipe, prep notes, and kitchen tips from the line.</p>
        </div>
